...
describe('Example', () => {
  ...
  describe('Positive', () => {
    ...
    test('Should handle valid GET request', async () => {
      const response = await fetch('http://localhost:8090/v1/user/retrieve?test=123&id=50');
      expect(response.status).toBe(200);
      const json = await response.json();
      expect(json).toEqual({
        status: 'success',
    ...
    test('Should send an image with a correct header', async () => {
      const response = await fetch('http://localhost:8090/v1/avatar/send?userId=123');
      expect(response.status).toBe(200);
    ...
    test('Should stream an image with a correct header', async () => {
      const response = await fetch('http://localhost:8090/v1/avatar/stream?userId=123');
      expect(response.status).toBe(200);
      expect(response.headers.has('Content-type')).toBeTruthy();
  ...
  describe('Negative', () => {
    test('GET request should fail on missing input param', async () => {
      const response = await fetch('http://localhost:8090/v1/user/retrieve?test=123');
    ...
    test('GET request should fail on specific value in handler implementation', async () => {
      const response = await fetch('http://localhost:8090/v1/user/retrieve?test=123&id=101');
