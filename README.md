...
The response schema generally may be just `z.string()`, but I made more specific `ez.download()` that also supports
`ez.download("binary")` and `ez.download("base64")` variants which are reflected in the
[generated documentation](#creating-a-documentation).

```typescript
const fileStreamingEndpointsFactory = new EndpointsFactory(
  new ResultHandler({
    positive: { schema: ez.download("buffer"), mimeType: "image/*" },
