...
import type { AbstractResultHandler } from "../src/result-handler.ts";
  ...
  describe("Wildcard status codes", () => {
    const getResponses = (
      ...
      new Documentation({
        ...
        routing: {
          v1: {
            mtpl: new EndpointsFactory(resultHandler).buildVoid({
              method: "post",
              handler: vi.fn(),
            }),
          },
        ...
        wildcardStatusCodes,
      }).getSpec().paths?.["/v1/mtpl"]?.post?.responses ?? {};

    test("should keep literal status codes when disabled", () => {
    ...
    test("should collapse the codes of a single schema into a wildcard when enabled", () => {
      const responses = getResponses(
        new ResultHandler({
          positive: { statusCode: [200, 201, 204], schema: z.literal("ok") },
          negative: { statusCode: 400, schema: z.literal("error") },
          handler: vi.fn(),
        }),
        true,
      ...
      const subject = responses["2XX"];
      expect(
        subject && "description" in subject ? subject.description : "",
      ).toBe("POST /v1/mtpl Positive response");
    });
    ...
    test("should collapse the codes of separate ApiResponse entries sharing the same schema", () => {
      ...
      const responses = getResponses(
        new ResultHandler({
          positive: [
            { statusCode: 200, schema: okSchema },
            { statusCode: 201, schema: okSchema },
          ],
          negative: { statusCode: 400, schema: z.literal("error") },
          handler: vi.fn(),
        }),
    ...
    test("should merge positive and negative variants independently", () => {
      const responses = getResponses(
        new ResultHandler({
          positive: { statusCode: [200, 201], schema: z.literal("ok") },
          negative: { statusCode: [400, 404], schema: z.literal("error") },
          handler: vi.fn(),
        }),
        true,
      );
      expect(Object.keys(responses).sort()).toEqual(["2XX", "4XX"]);
    });
    ...
    test("should keep non-collapsible literal codes alongside the wildcards", () => {
      ...
      const responses = getResponses(
        new ResultHandler({
          positive: { statusCode: [200, 201], schema: z.literal("ok") },
          negative: [
            { statusCode: [400, 404], schema: errA },
            { statusCode: 500, schema: errB },
          ],
          handler: vi.fn(),
        }),
        true,
      );
      expect(Object.keys(responses).sort()).toEqual(["2XX", "4XX", "500"]);
    });
    ...
    test("should keep literal codes when the range is claimed by another schema", () => {
      const responses = getResponses(
        new ResultHandler({
          ...
          negative: [
            { statusCode: 400, schema: z.literal("bad") },
            { statusCode: 401, schema: z.literal("bad") },
            { statusCode: 403, schema: z.literal("forbidden") },
          ],
          handler: vi.fn(),
        }),
        true,
    ...
    test("should keep literal codes spanning multiple ranges", () => {
      const responses = getResponses(
        new ResultHandler({
          positive: { statusCode: 200, schema: z.literal("ok") },
          negative: { statusCode: [400, 500], schema: z.literal("error") },
          handler: vi.fn(),
        }),
        true,
      );
    ...
    test("should not merge the codes sharing a schema but differing in MIME types", () => {
      ...
      const responses = getResponses(
        new ResultHandler({
          positive: [
            { statusCode: 200, schema: okSchema },
            { statusCode: 204, schema: okSchema, mimeType: null },
          ],
          negative: { statusCode: 400, schema: z.literal("error") },
          handler: vi.fn(),
        }),
    ...
    test("should pass the wildcard label to the descriptions callback", () => {
      const responses =
        new Documentation({
          ...
          descriptions: {
            positiveResponse: ({ statusCode }) =>
              `Custom response ${statusCode ?? "none"}`,
          },
        }).getSpec().paths?.["/v1/mtpl"]?.post?.responses ?? {};
      const subject = responses["2XX"];
      expect(
        subject && "description" in subject ? subject.description : "",
      ).toBe("Custom response 2XX");
    });
