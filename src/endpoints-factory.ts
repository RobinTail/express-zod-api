...
import {FlatObject, IO, Merge} from './helpers';
...
export class EndpointsFactory<mIN, mOUT> {
  ...
  public addMiddleware<IN extends IO, OUT extends FlatObject>(definition: MiddlewareDefinition<IN, mOUT, OUT>) {
  ...
  public build<IN extends IO, OUT extends IO>({methods, input, output, handler}: {
    methods: Method[],
    input: IN,
    output: OUT,
